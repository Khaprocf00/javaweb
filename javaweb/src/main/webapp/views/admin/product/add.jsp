...
<body>
	...
	<div class="container">
		<form action="" method="post" id="formSubmit"
			...
			<div class="form-group">
				<label for="imagePath">image product</label> <input type="file"
			...
			<div class="form-group">
				<label for="category">Choose Category</label> <select id="category"
					...
					<c:forEach var="item" items="${category}">
						<option <c:if test = "${product.categoryId == item.id }">selected="selected"</c:if> value="${item.id}">${item.name}</option>
					</c:forEach>
				</select>
			</div>
			<div class="form-group">
				<label for="brand">Choose Brand</label> <select id="brand"
					...
					<c:forEach var="item" items="${brand}">
						<option <c:if test = "${product.brandId == item.id }">selected="selected"</c:if> value="${item.id}">${item.name}</option>
					</c:forEach>
				</select>
			</div>
			<div class="form-group">
				<label for="slider">Choose Slider</label> <select id="slider"
					...
					<c:forEach var="item" items="${slider}">
						<option <c:if test = "${product.sliderId == item.id }">selected="selected"</c:if> value="${item.id}">${item.name}</option>
					</c:forEach>
				</select>
			</div>
			<div class="form-group">
				<label for="tag">Choose Tag</label> <select id="tag" name="tagId">
					...
					<c:forEach var="item" items="${tag}">
						<option <c:if test = "${product.tagId == item.id }">selected="selected"</c:if> value="${item.id}">${item.name}</option>
					</c:forEach>
				</select>
			</div>
			...
			<c:if test="${checkName == false}">
				<div class="alert alert-danger" role="alert">product này đã có
					rồi !</div>
			</c:if>
			<c:if test="${checkName == true}">
				<div class="alert alert-success" role="alert">Đã thêm thành
					công product " ${product.name}"</div>
			</c:if>
		</form>
	</div>
